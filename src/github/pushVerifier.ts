...
export async function verifyPush(
  ...
  // 5. Estimate lines changed.
  const linesEstimate = estimateLines(payload.commits);
  if (linesEstimate < MIN_LINES_CHANGED) {
    return {
      accepted: false,
  ...
  return {
    accepted: true,
    reason: `${payload.commits.length} commit(s), ~${linesEstimate} lines in ${meaningfulFiles.length} code file(s) on ${branch}`,
    linesEstimate,
  };
}
...
export async function recordVerifiedPush(
  ...
  const logEntry = JSON.stringify({
    ts: Date.now(),
    sha: payload.commits[0]?.id ?? "unknown",
